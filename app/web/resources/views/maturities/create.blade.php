...
<div class="min-h-screen bg-gray-50">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="bg-white shadow overflow-hidden sm:rounded-lg">
            <form action="{{ route('maturities.store') }}" method="POST" class="space-y-6 p-6">
                @csrf
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name') }}" required class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-theme-accent focus:border-theme-accent sm:text-sm" placeholder="e.g., Field A - Section 1">
                        @error('name')
                    ...
                    <div>
                        ...
                        <select name="top_sample_id" id="top_sample_id" required class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-theme-accent focus:border-theme-accent sm:text-sm">
                            ...
                            @foreach($samples as $sample)
                                <option value="{{ $sample->id }}" {{ old('top_sample_id') == $sample->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select name="bottom_sample_id" id="bottom_sample_id" required class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-theme-accent focus:border-theme-accent sm:text-sm">
                            ...
                            @foreach($samples as $sample)
                                <option value="{{ $sample->id }}" {{ old('bottom_sample_id') == $sample->id ? 'selected' : '' }}>
                                    {{ $sample->label }} - Brix: {{ number_format($sample->avg_brix, 2) }}
                                </option>
                            @endforeach
                        </select>
