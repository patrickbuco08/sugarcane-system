        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="mb-6">
                <a href="{{ route('dashboard') }}"
                    class="inline-flex items-center text-theme-primary hover:text-theme-secondary">
                ...
                <div class="px-4 py-5 sm:px-6">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <div class="mt-4 sm:mt-0">
                            <span
                                class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-theme-accent text-white">
                ...
                <div class="border-t border-gray-200 px-4 py-5 sm:p-0">
                    <dl class="sm:divide-y sm:divide-gray-200">
                            ...
                            <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div class="bg-gray-50 p-4 rounded-lg">
                                        ...
                                        <p class="mt-1 text-2xl font-semibold">{{ number_format($sample->avg_brix, 2) }}°
                                        </p>
                                    </div>
                                    <div class="bg-gray-50 p-4 rounded-lg">
                                ...
                                <form action="{{ route('samples.update-batch', $sample) }}" method="POST" class="max-w-md">
                                    ...
                                    <div class="flex space-x-2">
                                        <select name="harvest_batch_id" id="harvest_batch_id"
                                            class="block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-theme-accent focus:border-theme-accent sm:text-sm rounded-md">
                                            <option value="">-- Select a batch --</option>
                                            @foreach ($harvestBatches as $batch)
                                                <option value="{{ $batch->id }}"
                                                    {{ $sample->harvest_batch_id == $batch->id ? 'selected' : '' }}>
                                                    {{ $batch->label }} ({{ $batch->week_of->format('M d, Y') }}) -
                                                    {{ number_format($batch->tons_harvested, 1) }} tons
                                                </option>
                                            @endforeach
                                        </select>
                                        <button type="submit"
                                            class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-theme-primary hover:bg-theme-secondary focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-theme-accent">
                                            Update
                                        </button>
                                    </div>
                                </form>
                        ...
                        <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                            ...
                            <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                                {{ $sample->updated_at->format('M d, Y H:i:s') }}
                            </dd>
                        </div>
